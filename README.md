...
* [Godot 4.6](https://godotengine.org/releases/4.6) (exact version; C# support is not required),
...
* Change the current directory to the project's root directory:
  ...
  cd libre-nikki
  ```
* Before exporting the project, make sure that you have Godot export templates installed. If they are missing, download the export templates using `wget` and then extract them with `7z` to `$XDG_DATA_HOME/godot/export_templates/4.6.1.stable/`:
  ```sh
  wget https://github.com/godotengine/godot-builds/releases/download/4.6.1-stable/Godot_v4.6.1-stable_export_templates.tpz &&
  7z e Godot_v4.6.1-stable_export_templates.tpz -o${XDG_DATA_HOME:-$HOME/.local/share}/godot/export_templates/4.6.1.stable/ &&
  rm Godot_v4.6.1-stable_export_templates.tpz
  ```
* Export the project:
  ```sh
